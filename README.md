...
- ✅ 使用账号密码自动登录（登录即签到）
- ✅ 多账号支持：`ACCOUNTS` 多行配置，逐账号顺序签到，汇总 Telegram 通知
...
| Name | Value | 说明 |
|------|-------|------|
| `ACCOUNTS` | 多行文本 | 每行一个账号，格式 `邮箱:密码`，可配置任意多个账号 |

> **示例**（Secret 值中直接换行）：
> ```
> user1@qq.com:Pass1234
> user2@gmail.com:Abcd5678
> ```
> 密码中可以包含冒号，脚本只按第一个冒号分割邮箱与密码。
...
3. **多账号循环**：多个账号逐个顺序签到，单个账号失败不影响其他账号；任一失败时 Actions 退出码为 1
4. **双通道代理**：优先自有节点；未配置或失败时从免费代理池并发探测无 WAF 干净 IP 兜底（探测结果所有账号共享轮换使用）
